...
func TestSerialization(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "https://foo.com", strings.NewReader(`{
	  "action": "created",
	  "comment": {
	    ...
	    "body": "This is a really good change! 🔥 💣"
	  },
	  "repository": {
	    "id": 35129377,
	    "name": "public-repo",
	...
	event, err := ParseWebHook("commit_comment", payload)
	if err != nil {
		t.Fatal(err)
	}
	comment := event.(*CommitCommentEvent).Comment
	if comment.Body != "This is a really good change! 🔥 💣" {
		t.Fatal("wrong result expected for Body")
	}
}
